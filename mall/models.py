...
class Product(models.Model):
    ...
    desc = models.CharField('商品简介', max_length=256, null=True, blank=True)
    # content = models.TextField('商品描述')
    # 富文本
    content = RichTextField('商品描述')

    types = models.SmallIntegerField('商品类型', choices=constants.PRODUCT_TYPES_CHOICES,
                                     default=constants.PRODUCT_TYPE_ACTUAL)

    price = models.IntegerField('积分兑换价格')
    ...
    score = models.FloatField('商品的评分', default=0)
    is_valid = models.BooleanField('是否有效', default=True)
