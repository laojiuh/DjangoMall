...
def index(request):
    ...
    # 、在时间范围内的  start_time__lte=now_time, end_time__gte=now_time

    # 首页的新闻  置顶的、有效的  也可以添加上述的时间要求
    news_list = News.objects.filter(types=constants.NEWS_TYPE_NEW, is_top=True, is_valid=True)

    # 酒水推荐
    js_list = Product.objects.filter(status=constants.PRODUCT_STATUS_SELL,
                                     is_valid=True,
                                     tags__code='jstj')
    # 精选推荐
    jx_list = Product.objects.filter(status=constants.PRODUCT_STATUS_SELL,
                                     is_valid=True,
                                     tags__code='jxtj')
    # 从session中获取用户ID
